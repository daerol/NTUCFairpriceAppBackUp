...
open class BarcodeReaderCaptureViewController: RSCodeReaderViewController, DKImagePickerControllerCameraProtocol {
...
//                self.img = image!
                ...
                //  MARK:   Save the photo to user's album
    ...
    func setupUI() {
        ...
        let cameraSwitchButton: UIButton = {
            let cameraSwitchButton = UIButton()
            cameraSwitchButton.addTarget(self, action: #selector(switchCamera(_:)), for: .touchUpInside)
    ...
    func capturePhoto(sender: UIButton!) {
        ...
        self.imageOutput.captureStillImageAsynchronously (from: connection) {
            ...
            if sampleBuffer != nil {
                ...
                self.didFinishCapturingImage?(self.img!)
            } else {
                print("Error capturing photo: \(String(describing: error?.localizedDescription))")
            }
        }
